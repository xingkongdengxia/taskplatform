...
public class CaptchaUtil {
    ...
    private Font getFont(int size) {
        ...
        return font[randomFont.nextInt(5)];
    }
    ...
    public void write(HttpServletRequest request, HttpServletResponse response, HttpSession session, String key) throws IOException {
        //将生成的验证码放入Redis
        int timeOut = (int) session.getMaxInactiveInterval();
        log.debug("timeOut:" + timeOut);
        String sessionId = request.getSession().getId();//获取session的id  
        RedisUtil.set(sessionId + key, getCode(), timeOut);
        log.info("生成的验证码为：" + getCode());

        try {
            ByteArrayOutputStream tmp = new ByteArrayOutputStream();
            ImageIO.write(buffImg, "png", tmp);//将会值得图片输出到流  
            tmp.close();
