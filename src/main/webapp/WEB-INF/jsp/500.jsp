...
<html>
    ...
    <%
        /**
         * 监控出错人的IP String ip = request.getHeader(" x-forwarded-for"); if (ip ==
         * null || ip.length() == 0 || " unknown".equalsIgnoreCase(ip)) { ip =
         * request.getHeader(" Proxy-Client-IP"); // 获取代理ip } if (ip == null ||
         * ip.length() == 0 || " unknown".equalsIgnoreCase(ip)) { ip =
         * request.getHeader(" WL-Proxy-Client-IP"); // 获取代理ip } if (ip == null ||
         * ip.length() == 0 || " unknown".equalsIgnoreCase(ip)) { ip =
         * request.getRemoteAddr(); // 获取真实ip }
         * //out.println(ip+"<br/><br/>你的地址是：<br/><br/>");
         *
         * Document doc =
         * Jsoup.connect("http://ip.chinaz.com/?IP="+ip).timeout(9000).get();
         * Element e = doc.select("#status").first(); //out.println(e);
         */
    %>
</body>
</html>